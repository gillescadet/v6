...
public class RoomManager
{
    ...
    public RoomManager()
    {
        Rooms.SetRoomInviteNotificationCallback(LaunchedFromAcceptingInviteCallback);
    ...
    void GetRoomInfoCallback(Message<Room> msg)
    {
        ...
        Invite invite = new Invite(msg.Data.ID, msg.Data.Owner.OculusID);
    ...
    void JoinRoomCallback(Message<Room> msg)
    {
        ...
        Debug.LogFormat("Joined room: {0} owner: {1} count: ",
            msg.Data.ID, msg.Data.Owner.OculusID, msg.Data.Users.Count);

        m_roomID = msg.Data.ID;

        // if the caller left while I was in the process of joining, just hangup
        if (msg.Data.Users.Count != 2)
        {
