...
void Sample_Unpack( Sample s, out uint3 coords, out uint mip, out uint3 color, out uint occupancy )
{
	coords.x = s.row0 >> 20;
...
void Sample_UnpackColorAndOccupancy( Sample s, out uint3 color, out  uint occupancy )
{
	uint3 coords;
	uint mip;
	Sample_Unpack( s, coords, mip, color, occupancy );
