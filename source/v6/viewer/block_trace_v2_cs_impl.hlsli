...
void ProcessCell( uint cellRGBA, uint cellPos, CellParams cellParams )
{
	const uint multiSampledPixelWith = 2 * HLSL_CELL_SUPER_SAMPLING_WIDTH + 1;
...

	{
		...
		multiSampledMinPixelCoords = int2( multiSampledPixelPos ) - HLSL_CELL_SUPER_SAMPLING_WIDTH;
		minPixelCoords = int2( (multiSampledMinPixelCoords + 0.5f) / float( HLSL_CELL_SUPER_SAMPLING_WIDTH ) );
	...
	{
		const float rayEndVSX = mad( multiSampledMinPixelCoords.x + 0.5f, c_blockMultiSampledScreenToClipScale.x, c_blockScreenToClipOffset.x );
		const float rayEndVSY = mad( multiSampledMinPixelCoords.y + 0.5f, c_blockMultiSampledScreenToClipScale.y, c_blockScreenToClipOffset.y );
		const float3 rayEndVS0 = float3( rayEndVSX, rayEndVSY, -c_blockZNear );
		const float3 rayEndVS1 = float3( rayEndVSX + c_blockMultiSampledScreenToClipScale.x, rayEndVSY, -c_blockZNear );
		const float3 rayEndVS2 = float3( rayEndVSX, rayEndVSY  + c_blockMultiSampledScreenToClipScale.y, -c_blockZNear );
		...
		for ( uint y = 0; y < multiSampledPixelWith; ++y )
		{
			...
			const uint hitYMod3_x_3 = Mod3( hitY ) * HLSL_CELL_SUPER_SAMPLING_WIDTH;
			...
			for ( uint x = 0; x < multiSampledPixelWith; ++x, rayDir += rayEndWSR )
			{
				...
				const uint hitShift = Div3( hitX ) * HLSL_CELL_SUPER_SAMPLING_WIDTH_SQ + hitYMod3_x_3 + Mod3( hitX );
...

	{
		...
		for ( uint y = 0; y < 3; ++y )
		{
			for ( uint x = 0; x < 3; ++x )
			{
				const uint pixelOccupancy = (hitBits[y] >> (x * HLSL_CELL_SUPER_SAMPLING_WIDTH * 3)) & pixelOccupancyMask;
