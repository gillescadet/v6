...
struct GPUCaptureResources_s
{
	...
	GPUBuffer_s					samples;
	GPUBuffer_s					sampleInfo;
	GPUBuffer_s					sampleNodeOffsets;
	GPUBuffer_s					firstChildOffsetPages[HLSL_OCTREE_FIRST_CHILD_OFFSET_PAGE_MAX_COUNT];
	ID3D11UnorderedAccessView*	firstChildOffsetsLimitedUAV;
	GPUBuffer_s					leafPages[HLSL_OCTREE_LEAF_PAGE_MAX_COUNT];
	GPUBuffer_s					octreeInfo;
	GPUBuffer_s					blockPos;
	GPUBuffer_s					blockData;
	GPUBuffer_s					blockInfo;
...
static void ClearNode( CaptureContext_s* captureContext )
{
	...
	u32 values[4] = {};
	g_deviceContext->ClearUnorderedAccessViewUint( res->octreeInfo.uav, values );
	g_deviceContext->ClearUnorderedAccessViewUint( res->firstChildOffsetsLimitedUAV, values );
}

static u32 Collect_Mip( const CaptureContext_s* captureContext, const Vec3* samplePos, const Vec3 basis[3], ID3D11ShaderResourceView* colorSRV, ID3D11ShaderResourceView* depthSRV, bool isSRGB )
{
	...
	// Clear
	u32 values[4] = {};
	g_deviceContext->ClearUnorderedAccessViewUint( res->sampleInfo.uav, values );
...

	{
		...
		cbSample->c_sampleGammaCorrection = isSRGB ? (1.0f / 2.2f) : 1.0f;
	...
	g_deviceContext->CSSetUnorderedAccessViews( HLSL_SAMPLE_INFO_SLOT, 1, &res->sampleInfo.uav, nullptr );
	g_deviceContext->CSSetShader( res->computeCollect[captureContext->desc.logReadBack].m_computeShader, nullptr, 0 );

	// Dispatch
	V6_ASSERT_TXT( (captureContext->desc.samplingWidth & 7) == 0, String_Format( "Sampling width should be a multiple of 8 (actual value is %d)", captureContext->desc.samplingWidth ) ); 
	...
	g_deviceContext->CSSetUnorderedAccessViews( HLSL_SAMPLE_INFO_SLOT, 1, (ID3D11UnorderedAccessView**)nulls, nullptr );

	GPUEvent_End();

	const hlsl::SampleInfo* sampleInfo = (hlsl::SampleInfo*)GPUBuffer_MapReadBack( &res->sampleInfo );
	const u32 sampleCount = sampleInfo->count;
	V6_ASSERT( sampleCount <= captureContext->resSampleCount );

	if ( captureContext->desc.logReadBack )
	{
		// Read back
		V6_DEVMSG( "\n" );
		ReadBack_Log( "sample", sampleCount, "count" );
#if HLSL_DEBUG_COLLECT == 1
		ReadBack_Log( "sample", sampleInfo->pixelCount, "pixelCount" );
		ReadBack_Log( "sample", sampleInfo->pixelSampleCount, "pixelSampleCount" );
		ReadBack_Log( "sample", sampleInfo->out, "out" );
		ReadBack_Log( "sample", sampleInfo->error, "error" );
		V6_ASSERT( sampleInfo->error == 0 );
#endif // #if HLSL_DEBUG_COLLECT == 1
	}

	GPUBuffer_UnmapReadBack( &res->sampleInfo );
...
static u32 Collect_Onion( const CaptureContext_s* captureContext, const Vec3* samplePos, const Vec3 basis[3], ID3D11ShaderResourceView* colorSRV, ID3D11ShaderResourceView* depthSRV, bool isSRGB )
{
	GPUEvent_Begin( s_gpuEventCollect );
...

	{
		...
		cbSample->c_sampleOnionGammaCorrection = isSRGB ? (1.0f / 2.2f) : 1.0f;
...
static u32 Collect( const CaptureContext_s* captureContext, const Vec3* samplePos, const Vec3 basis[3], ID3D11ShaderResourceView* colorSRV, ID3D11ShaderResourceView* depthSRV, bool isSRGB )
{
	u32 sampleCount = 0;
	if ( captureContext->desc.movingPointOfView )
		sampleCount = Collect_Mip( captureContext, samplePos, basis, colorSRV, depthSRV, isSRGB );
	else
		sampleCount = Collect_Onion( captureContext, samplePos, basis, colorSRV, depthSRV, isSRGB );
	return sampleCount;
}

static Vec3u ComputeSampleDispatchGroupCount( u32 sampleCount )
{
	sampleCount = Max( 1u , sampleCount );

	const u32 lineCount = sampleCount / HLSL_SAMPLE_THREAD_MAX_COUNTX;
	const u32 remainder = sampleCount - lineCount * HLSL_SAMPLE_THREAD_MAX_COUNTX;

	Vec3u dispatchGroupCount;
	dispatchGroupCount.x = lineCount == 0 ? Max( 1u, HLSL_GROUP_COUNT( remainder, HLSL_SAMPLE_THREAD_GROUP_SIZEX ) ) : HLSL_GROUP_COUNT( HLSL_OCTREE_THREAD_MAX_COUNTX, HLSL_SAMPLE_THREAD_GROUP_SIZEX );
	dispatchGroupCount.y = Max( 1u, HLSL_GROUP_COUNT( lineCount, HLSL_SAMPLE_THREAD_GROUP_SIZEY ) );
	dispatchGroupCount.z = 1;

	V6_ASSERT( dispatchGroupCount.x < 65536 );
	V6_ASSERT( dispatchGroupCount.y < 65536 );
	V6_ASSERT( dispatchGroupCount.z < 65536 );

	return dispatchGroupCount;
}

static Vec3u ComputeLeafDispatchGroupCount( u32 leafCount )
{
	const u32 lineCount = leafCount / HLSL_OCTREE_THREAD_MAX_COUNTX;
	const u32 remainder = leafCount - lineCount * HLSL_OCTREE_THREAD_MAX_COUNTX;

	Vec3u dispatchGroupCount;
	dispatchGroupCount.x = lineCount == 0 ? Max( 1u, HLSL_GROUP_COUNT( remainder, HLSL_OCTREE_THREAD_GROUP_SIZEX ) ) : HLSL_GROUP_COUNT( HLSL_OCTREE_THREAD_MAX_COUNTX, HLSL_OCTREE_THREAD_GROUP_SIZEX );
	dispatchGroupCount.y = Max( 1u, HLSL_GROUP_COUNT( lineCount, HLSL_OCTREE_THREAD_GROUP_SIZEY ) );
	dispatchGroupCount.z = 1;
...
static u32 BuildNode( CaptureContext_s* captureContext, u32 sampleCount )
{
	GPUEvent_Begin( s_gpuEventBuildNode );

	GPUCaptureResources_s* res = captureContext->res;

	ID3D11UnorderedAccessView* firstChildOffsetUAVs[8] = {};
	for ( u32 octreeNodePage = 0; octreeNodePage < captureContext->resNodePageCount; ++octreeNodePage )
		firstChildOffsetUAVs[octreeNodePage] = res->firstChildOffsetPages[octreeNodePage].uav;

	ID3D11UnorderedAccessView* leafUAVs[8] = {};
	for ( u32 octreeLeafPage = 0; octreeLeafPage < captureContext->resLeafPageCount; ++octreeLeafPage )
		leafUAVs[octreeLeafPage] = res->leafPages[octreeLeafPage].uav;
	...
	g_deviceContext->CSSetShaderResources( HLSL_SAMPLE_INFO_SLOT, 1, &res->sampleInfo.srv );
	g_deviceContext->CSSetUnorderedAccessViews( HLSL_OCTREE_SAMPLE_NODE_OFFSET_SLOT, 1, &res->sampleNodeOffsets.uav, nullptr );
	g_deviceContext->CSSetUnorderedAccessViews( HLSL_OCTREE_FIRST_CHILD_OFFSET_SLOT0, captureContext->resNodePageCount, firstChildOffsetUAVs, nullptr );
	g_deviceContext->CSSetUnorderedAccessViews( HLSL_OCTREE_LEAF_SLOT0, captureContext->resLeafPageCount, leafUAVs, nullptr );
	g_deviceContext->CSSetUnorderedAccessViews( HLSL_OCTREE_INFO_SLOT, 1, &res->octreeInfo.uav, nullptr );
	g_deviceContext->CSSetShader( res->computeBuildInner.m_computeShader, nullptr, 0 );

	const Vec3u sampleDispatchGroupCount = ComputeSampleDispatchGroupCount( sampleCount );
	...
	for ( u32 level = 0; level < levelCount; ++level )
	{
		...
		// Dispatch
		g_deviceContext->Dispatch( sampleDispatchGroupCount.x, sampleDispatchGroupCount.y, sampleDispatchGroupCount.z );
	}

	// Unset
	static const void* nulls[8] = {};
	g_deviceContext->CSSetShaderResources( HLSL_SAMPLE_SLOT, 1, (ID3D11ShaderResourceView**)nulls);
	g_deviceContext->CSSetShaderResources( HLSL_SAMPLE_INFO_SLOT, 1, (ID3D11ShaderResourceView**)nulls);
	g_deviceContext->CSSetUnorderedAccessViews( HLSL_OCTREE_SAMPLE_NODE_OFFSET_SLOT, 1, (ID3D11UnorderedAccessView**)nulls, nullptr );
	g_deviceContext->CSSetUnorderedAccessViews( HLSL_OCTREE_FIRST_CHILD_OFFSET_SLOT0, captureContext->resNodePageCount, (ID3D11UnorderedAccessView**)nulls, nullptr );
	g_deviceContext->CSSetUnorderedAccessViews( HLSL_OCTREE_LEAF_SLOT0, captureContext->resLeafPageCount, (ID3D11UnorderedAccessView**)nulls, nullptr );
	g_deviceContext->CSSetUnorderedAccessViews( HLSL_OCTREE_INFO_SLOT, 1, (ID3D11UnorderedAccessView**)nulls, nullptr );

	GPUEvent_End();

	const hlsl::OctreeInfo* octreeInfo = (hlsl::OctreeInfo*)GPUBuffer_MapReadBack( &res->octreeInfo );

	const u32 nodeCount = CODEC_GRID_MAX_COUNT * 8 + octreeInfo->nodeCount;
	V6_ASSERT_TXT( nodeCount <= captureContext->resNodeCount, String_Format( "Too many nodes: %d/%d (increase average layer count)", nodeCount, captureContext->resNodeCount ) );

	const u32 leafCount = octreeInfo->leafCount;
	V6_ASSERT_TXT( leafCount <= captureContext->resLeafCount, String_Format( "Too many leaves: %d/%d (increase average layer count)", leafCount, captureContext->resLeafCount ) );

	if ( captureContext->desc.logReadBack )
	{
		V6_DEVMSG( "\n" );
		ReadBack_Log( "octreeContext", nodeCount, "nodeCount" );
		ReadBack_Log( "octreeContext", leafCount, "leafCount" );
	}

	GPUBuffer_UnmapReadBack( &res->octreeInfo );

	return leafCount;
}

static void FillLeaf( CaptureContext_s* captureContext, u32 sampleCount )
{
	...
	ID3D11ShaderResourceView* firstChildOffsetSRVs[8] = {};
	for ( u32 octreeNodePage = 0; octreeNodePage < captureContext->resNodePageCount; ++octreeNodePage )
		firstChildOffsetSRVs[octreeNodePage] = res->firstChildOffsetPages[octreeNodePage].srv;
	...
	g_deviceContext->CSSetShaderResources( HLSL_OCTREE_FIRST_CHILD_OFFSET_SLOT0, captureContext->resNodePageCount, firstChildOffsetSRVs );
	g_deviceContext->CSSetUnorderedAccessViews( HLSL_OCTREE_LEAF_SLOT0, captureContext->resLeafPageCount, leafUAVs, nullptr );

	// Dispatch
	const Vec3u sampleDispatchGroupCount = ComputeSampleDispatchGroupCount( sampleCount );
	GPUCompute_Dispatch( &res->computeFillLeaf, sampleDispatchGroupCount.x, sampleDispatchGroupCount.y, sampleDispatchGroupCount.z );
	...
	g_deviceContext->CSSetShaderResources( HLSL_OCTREE_FIRST_CHILD_OFFSET_SLOT0, captureContext->resNodePageCount, (ID3D11ShaderResourceView**)nulls);
	g_deviceContext->CSSetUnorderedAccessViews( HLSL_OCTREE_LEAF_SLOT0, captureContext->resLeafPageCount, (ID3D11UnorderedAccessView**)nulls, nullptr );
...
static u32 PackColor( CaptureContext_s* captureContext )
{
	...
	// Clear
	u32 values[4] = {};
	g_deviceContext->ClearUnorderedAccessViewUint( res->blockInfo.uav, values );

	ID3D11ShaderResourceView* firstChildOffsetSRVs[8] = {};
	for ( u32 octreeNodePage = 0; octreeNodePage < captureContext->resNodePageCount; ++octreeNodePage )
		firstChildOffsetSRVs[octreeNodePage] = res->firstChildOffsetPages[octreeNodePage].srv;

	ID3D11ShaderResourceView* leafSRVs[8] = {};
	for ( u32 octreeLeafPage = 0; octreeLeafPage < captureContext->resLeafPageCount; ++octreeLeafPage )
		leafSRVs[octreeLeafPage] = res->leafPages[octreeLeafPage].srv;

	// Set
	g_deviceContext->CSSetConstantBuffers( v6::hlsl::CBOctreeSlot, 1, &res->cbOctree.buf );
	g_deviceContext->CSSetShaderResources( HLSL_OCTREE_FIRST_CHILD_OFFSET_SLOT0, captureContext->resNodePageCount, firstChildOffsetSRVs );
	g_deviceContext->CSSetShaderResources( HLSL_OCTREE_LEAF_SLOT0, captureContext->resLeafPageCount, leafSRVs );
	g_deviceContext->CSSetShaderResources( HLSL_OCTREE_INFO_SLOT, 1, &res->octreeInfo.srv );
	...
	g_deviceContext->CSSetUnorderedAccessViews( HLSL_BLOCK_INFO_SLOT, 1, &res->blockInfo.uav, nullptr );
	g_deviceContext->CSSetShader( res->computePackColor.m_computeShader, nullptr, 0 );

	const Vec3u leafDispatchGroupCount = ComputeLeafDispatchGroupCount( captureContext->frameState.leafCount );
	...
	for ( u32 bucket = 0; bucket < HLSL_BUCKET_COUNT; ++bucket )
	{
		...
		// Dispatch
		g_deviceContext->Dispatch( leafDispatchGroupCount.x, leafDispatchGroupCount.y, leafDispatchGroupCount.z );
	}

	// Unset
	static const void* nulls[8] = {};
	g_deviceContext->CSSetShaderResources( HLSL_OCTREE_FIRST_CHILD_OFFSET_SLOT0, captureContext->resNodePageCount, (ID3D11ShaderResourceView**)nulls);
	g_deviceContext->CSSetShaderResources( HLSL_OCTREE_LEAF_SLOT0, captureContext->resLeafPageCount, (ID3D11ShaderResourceView**)nulls);
	g_deviceContext->CSSetShaderResources( HLSL_OCTREE_INFO_SLOT, 1, (ID3D11ShaderResourceView**)nulls);
	...
	g_deviceContext->CSSetUnorderedAccessViews( HLSL_BLOCK_INFO_SLOT, 1, (ID3D11UnorderedAccessView**)nulls, nullptr );

	GPUEvent_End();

	const hlsl::BlockInfo* blockInfo = (hlsl::BlockInfo*)GPUBuffer_MapReadBack( &res->blockInfo );
	...
	for ( u32 bucket = 0; bucket < HLSL_BUCKET_COUNT; ++bucket )
	{
		if ( blockInfo->counts[bucket] == 0 )
			continue;

		static const u32 cellPerBucketCounts[] = { 4, 8, 16, 32, 64 };
		const u32 maxCellCount = blockInfo->counts[bucket] * cellPerBucketCounts[bucket];

		if ( captureContext->desc.logReadBack )
		{
			V6_DEVMSG( "\n" );
			ReadBack_Log( "blockContext", bucket, "bucket" );
			ReadBack_Log( "blockContext", blockInfo->counts[bucket], "blockCount" );
			ReadBack_Log( "blockContext", blockInfo->posOffsets[bucket], "posOffset" );
			ReadBack_Log( "blockContext", blockInfo->dataOffsets[bucket], "dataOffset" );
			ReadBack_Log( "blockContext", blockInfo->cellCounts[bucket], "realCellCount" );
			ReadBack_Log( "blockContext", maxCellCount, "maxCellCount" );
		}

		allBlockCount += blockInfo->counts[bucket];
		allRealCellCount += blockInfo->cellCounts[bucket];
		allMaxCellCount += maxCellCount;
	}
	
	ReadBack_Log( "blockContext", blockInfo->minNullLeafID, "minNullLeafID" );
	ReadBack_Log( "blockContext", blockInfo->maxNullLeafID, "maxNullLeafID" );
	V6_ASSERT_TXT( blockInfo->minNullLeafID == 0xFFFFFFFF, String_Format( "Null leaf ID: %d..%d, leaf count: %d/%d (unexpected error)", blockInfo->minNullLeafID, blockInfo->maxNullLeafID, allRealCellCount, captureContext->resLeafCount ) );

	ReadBack_Log( "blockContext", blockInfo->minOverLeafID, "minOverLeafID" );
	ReadBack_Log( "blockContext", blockInfo->maxOverLeafID, "maxOverLeafID" );
	V6_ASSERT_TXT( blockInfo->minOverLeafID == 0xFFFFFFFF, String_Format( "Over leaf ID: %d..%d, leaf count: %d/%d (unexpected error)", blockInfo->minOverLeafID, blockInfo->maxOverLeafID, allRealCellCount, captureContext->resLeafCount ) );
	...
	V6_ASSERT_TXT( allBlockCount <= captureContext->resBlockPosCount, String_Format( "Too many blocks: %d/%d", allBlockCount, captureContext->resBlockPosCount ) );
	V6_ASSERT_TXT( allMaxCellCount <= captureContext->resBlockDataCount, String_Format( "Too many cells: %d/%d", allMaxCellCount, captureContext->resBlockDataCount ) );

	GPUBuffer_UnmapReadBack( &res->blockInfo );
...
void CaptureContext_Create( CaptureContext_s* captureContext, const CaptureDesc_s* desc )
{
	...
	V6_ASSERT_TXT( desc->gridWidth > 0 && (desc->gridWidth & 7) == 0, String_Format( "Grid width should be a multiple of 8 (actual value is %d)", desc->gridWidth ) );
	...
	captureContext->resLeafCount = (u32)(desc->gridWidth * desc->gridWidth * 6 * desc->averageLayerCount);
	captureContext->resLeafPageCount = (captureContext->resLeafCount + HLSL_OCTREE_LEAF_MAX_COUNT_PER_PAGE - 1) >> HLSL_OCTREE_LEAF_SHIFT;
	V6_ASSERT( captureContext->resLeafPageCount > 0 && captureContext->resLeafPageCount <= HLSL_OCTREE_LEAF_PAGE_MAX_COUNT );
	captureContext->resNodeCount = captureContext->resLeafCount * 5 / 2;
	captureContext->resNodePageCount = (captureContext->resNodeCount + HLSL_OCTREE_FIRST_CHILD_OFFSET_MAX_COUNT_PER_PAGE - 1) >> HLSL_OCTREE_FIRST_CHILD_OFFSET_SHIFT;
	V6_ASSERT( captureContext->resNodePageCount > 0 && captureContext->resNodePageCount <= HLSL_OCTREE_FIRST_CHILD_OFFSET_PAGE_MAX_COUNT );
...
#endif
	GPUBuffer_CreateStructured( &res->sampleInfo, sizeof( hlsl::SampleInfo ), 1, GPUBUFFER_CREATION_FLAG_READ_BACK, "sampleInfo" );
	GPUBuffer_CreateTyped( &res->sampleNodeOffsets, DXGI_FORMAT_R32_UINT, captureContext->resSampleCount, 0, "octreeSampleNodeOffsets" );
	
	V6_STATIC_ASSERT( HLSL_OCTREE_FIRST_CHILD_OFFSET_MAX_COUNT_PER_PAGE * sizeof( hlsl::uint ) <= MulMB_ConstExpr( 512u ) );
	for ( u32 octreeNodePage = 0; octreeNodePage < captureContext->resNodePageCount; ++octreeNodePage )
		GPUBuffer_CreateTyped( &res->firstChildOffsetPages[octreeNodePage], DXGI_FORMAT_R32_UINT, HLSL_OCTREE_FIRST_CHILD_OFFSET_MAX_COUNT_PER_PAGE, 0, String_Format( "octreeFirstChildOffsetPage%d", octreeNodePage ) );
	
	{
		...
		V6_ASSERT_D3D11( g_device->CreateUnorderedAccessView( res->firstChildOffsetPages[0].buf, &uavDesc, &res->firstChildOffsetsLimitedUAV ) );
	}
	
	V6_STATIC_ASSERT( sizeof( hlsl::OctreeLeaf ) == sizeof( hlsl::OctreeLeafOnion ) );
	// V6_STATIC_ASSERT( HLSL_OCTREE_LEAF_MAX_COUNT_PER_PAGE * sizeof( hlsl::OctreeLeaf ) <= MulMB_ConstExpr( 512u ) );
	for ( u32 octreeLeafPage = 0; octreeLeafPage < captureContext->resLeafPageCount; ++octreeLeafPage )
		GPUBuffer_CreateStructured( &res->leafPages[octreeLeafPage], sizeof( hlsl::OctreeLeaf ), HLSL_OCTREE_LEAF_MAX_COUNT_PER_PAGE, 0, String_Format( "octreeLeafPage%d", octreeLeafPage ) );
	
	GPUBuffer_CreateStructured( &res->octreeInfo, sizeof( hlsl::OctreeInfo ), 1, GPUBUFFER_CREATION_FLAG_READ_BACK, "octreeInfo" );
	...
	GPUBuffer_CreateStructured( &res->blockInfo, sizeof( hlsl::BlockInfo ), 1, GPUBUFFER_CREATION_FLAG_READ_BACK, "blockInfo" );

	if ( !desc->movingPointOfView )
	{
		GPUBuffer_CreateStructured( &res->sampleDebug, sizeof( hlsl::SampleDebugOnion ), 1, GPUBUFFER_CREATION_FLAG_READ_BACK, "sampleDebug" );
		hlsl::SampleDebugOnion sampleDebugDefault = {};
		sampleDebugDefault.minBlockCoords = Vec3u_Make( 0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF );
		GPUBuffer_CreateStructuredWithStaticData( &res->sampleDebugDefault, &sampleDebugDefault, sizeof( hlsl::SampleDebugOnion ), 1, 0, "sampleDebugDefault" );
	}
...
void CaptureContext_Release( CaptureContext_s* captureContext )
{
	...
	GPUBuffer_Release( &res->sampleInfo );
	GPUBuffer_Release( &res->sampleNodeOffsets );
	for ( u32 octreeNodePage = 0; octreeNodePage < captureContext->resNodePageCount; ++octreeNodePage )
		GPUBuffer_Release( &res->firstChildOffsetPages[octreeNodePage] );
	V6_RELEASE_D3D11( res->firstChildOffsetsLimitedUAV );
	for ( u32 octreeLeafPage = 0; octreeLeafPage < captureContext->resLeafPageCount; ++octreeLeafPage )
		GPUBuffer_Release( &res->leafPages[octreeLeafPage] );
	GPUBuffer_Release( &res->octreeInfo );
	GPUBuffer_Release( &res->blockPos );
	GPUBuffer_Release( &res->blockData );
	GPUBuffer_Release( &res->blockInfo );
...
void CaptureContext_Begin( CaptureContext_s* captureContext, const Vec3* origin )
{
	...
	captureContext->frameState.leafCount = 0;
}
...
{
	const u32 sampleCount = Collect( captureContext, samplePos, basis, (ID3D11ShaderResourceView*)colorView, (ID3D11ShaderResourceView*)depthView, isSRGB );
	captureContext->frameState.leafCount = BuildNode( captureContext, sampleCount );
	FillLeaf( captureContext, sampleCount );
	return captureContext->frameState.leafCount;
}
...
void CaptureContext_MapBlocksForRead( CaptureContext_s* captureContext, u32* blockCounts, void** blockPos, void** blockData )
{
...

	{
		...
		for ( u32 bucket = 0; bucket < HLSL_BUCKET_COUNT; ++bucket )
			blockCounts[bucket] = blockInfo->counts[bucket];
